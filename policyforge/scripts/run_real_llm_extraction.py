#!/usr/bin/env python3
"""
Real LLM Extraction - Using Mistral API on Actual Policy Files

This script demonstrates the core value proposition: automated extraction.
"""

import sys
sys.path.insert(0, '.')
import json
import requests
import time
from pathlib import Path
from src.schema import PolicyCriteria

API_KEY = "f7aoZhlIpeTRvtIuee2pqSrppCFKhGQ3"
API_URL = "https://api.mistral.ai/v1/chat/completions"

def extract_with_llm(policy_text: str, policy_id: str) -> dict:
    """Use Mistral LLM to extract policy criteria."""
    
    prompt = f"""You are a Medicare policy analyst. Extract structured criteria from this policy text.

Policy ID: {policy_id}
Policy Text:
{policy_text[:4000]}

Extract these fields:
1. frequency_limit_months: How often can the service be billed? (12 for annual, 24 for biennial, null for as-needed)
2. target_hcpcs_codes: List of HCPCS/CPT codes covered (e.g., ["G0103", "77067"])
3. age_min: Minimum age for coverage (null if none)
4. age_max: Maximum age for coverage (null if none)
5. eligible_conditions: List of medical conditions that qualify (brief)
6. exclusions: List of exclusions (brief)

Return ONLY valid JSON in this format:
{{
  "policy_id": "{policy_id}",
  "frequency_limit_months": 12,
  "target_hcpcs_codes": ["code1", "code2"],
  "age_min": 50,
  "age_max": null,
  "eligible_conditions": ["condition description"],
  "eligible_icd10_diagnoses": [],
  "exclusions": ["exclusion description"],
  "citations": []
}}"""

    try:
        response = requests.post(
            API_URL,
            headers={
                "Content-Type": "application/json",
                "Authorization": f"Bearer {API_KEY}"
            },
            json={
                "model": "mistral-large-latest",
                "messages": [{"role": "user", "content": prompt}],
                "response_format": {"type": "json_object"},
                "temperature": 0.1
            },
            timeout=30
        )
        
        if response.status_code == 429:
            print(f"  Rate limited, waiting 60s...")
            time.sleep(60)
            return extract_with_llm(policy_text, policy_id)
        
        response.raise_for_status()
        result = response.json()
        content = result['choices'][0]['message']['content']
        extracted = json.loads(content)
        
        return extracted
        
    except Exception as e:
        print(f"  ❌ Error: {str(e)}")
        return None

def main():
    print('='*80)
    print('REAL LLM EXTRACTION - Demonstrating Automation Value')
    print('='*80)
    print()
    
    # Policy files to extract
    policies_to_extract = [
        ('data/policies/NCD_220.4_Mammography.txt', 'NCD_220.4'),
        ('data/policies/NCD_210.1_PSA_Screening.txt', 'NCD_210.1'),
    ]
    
    results = []
    
    for policy_path, policy_id in policies_to_extract:
        print(f'Extracting: {policy_id}')
        
        # Read policy text
        with open(policy_path) as f:
            policy_text = f.read()
        
        # Extract with LLM
        print(f'  🤖 Running LLM extraction...')
        extracted = extract_with_llm(policy_text, policy_id)
        
        if extracted:
            # Save result
            output_path = f'data/policies/{policy_id}_extracted_LLM.json'
            with open(output_path, 'w') as f:
                json.dump(extracted, f, indent=2)
            
            print(f'  ✅ Extracted: {extracted["target_hcpcs_codes"]}, freq={extracted["frequency_limit_months"]} months')
            results.append({
                'policy_id': policy_id,
                'success': True,
                'hcpcs': extracted['target_hcpcs_codes'],
                'frequency': extracted['frequency_limit_months']
            })
        else:
            print(f'  ❌ Extraction failed')
            results.append({
                'policy_id': policy_id,
                'success': False
            })
        
        print()
        time.sleep(2)  # Rate limiting
    
    print('='*80)
    print(f'COMPLETED: {sum(1 for r in results if r["success"])}/{len(results)} successful extractions')
    print('='*80)
    print()
    print('Next step: Compare LLM extraction vs manual extraction to measure accuracy')
    
    return results

if __name__ == '__main__':
    main()
